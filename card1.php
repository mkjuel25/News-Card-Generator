        ...
        document.getElementById('urlForm').addEventListener('submit', async function(event) {
            ...
            try {
                ...
                if (data.success) {
                    ...
                    const cardHtml = `
                        <div id="${cardId}" class="bg-gray-100 overflow-hidden transform transition duration-300 hover:scale-105">
                            ...
                            <div class="p-6">
                                <h2 class="text-lg font-bold text-gray-800 mb-2 leading-tight line-clamp-2">${displayHeadline}</h2>
                                <a href="${data.original_url}" target="_blank" rel="noopener noreferrer"
                                   class="inline-flex items-center text-blue-600 hover:text-blue-800 transition duration-150 ease-in-out font-semibold group">
                                   <img src="${data.favicon}" alt="Favicon" class="w-4 h-4 mr-2">
                                   ${data.website_name}
                                   <svg class="ml-2 w-4 h-4 transition-transform duration-150 group-hover:translate-x-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
